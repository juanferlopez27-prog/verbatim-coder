...
async function callGemini(apiKey, prompt, retries = 3) {
  ...
  for (let attempt = 0; attempt < retries; attempt++) {
    ...
    if (res.status === 429) {
      if (attempt < retries - 1) { await new Promise(r => setTimeout(r, 65000)); continue; }
      throw new Error("Límite de requests alcanzado después de varios intentos. Intenta con menos respuestas.");
    }
    if (!res.ok) {
      const err = await res.json().catch(() => ({}));
      const msg = err?.error?.message || `Error ${res.status}`;
      if (res.status === 400) throw new Error("API Key inválida. Verifica que copiaste bien tu clave.");
      throw new Error(msg);
    }
...
export default function App() {
  ...
  const estSecs = batchCount * 20 + 15;
...
2. Nombres cortos (2-4 palabras), descriptivos y sustantivos.
3. SIEMPRE incluir "OTRO / NO CLASIFICABLE" como última categoría.
MUESTRA (${sample.length} respuestas):
${sample.map((r, i) => `${i + 1}. ${r}`).join("\n")}
Responde ÚNICAMENTE con este JSON:
{"codebook":[{"id":"C01","name":"Nombre corto","description":"Descripción"}]}`;
...
CONTEXTO: Pregunta: ${question || "(no especificada)"} · Idioma: ${lang}
LIBRO DE CÓDIGOS:
${cbStr}
REGLAS:
1. Asigna 1 a 3 códigos por respuesta, SOLO del libro de códigos.
2. Si no encaja usa "OTRO / NO CLASIFICABLE".
3. Sentimiento: POSITIVO, NEGATIVO, NEUTRO o MIXTO.
4. Nota (máx 8 palabras) solo si es ambiguo.
RESPUESTAS:
...
{"coded":[{"id":1,"codes":["C01"],"sentiment":"POSITIVO","note":""}]}`;
  ...
  const runCoding = useCallback(async () => {
    if (!apiKey.trim()) { setError("Ingresa tu API Key de Google AI Studio."); return; }
    if (responses.length === 0) { setError("Pega al menos algunas respuestas."); return; }
    ...
    try {
      ...
      for (let b = 0; b < batches.length; b++) {
        if (cancelRef.current) return;
        setProgress({
          current: b + 1, total: batches.length,
          msg: `Codificando lote ${b + 1} de ${batches.length} · ${Math.min((b + 1) * BATCH_SIZE, responses.length)} / ${responses.length} respuestas…`,
        });
  ...
  }, [apiKey, rawText, question, sector, lang, maxcats, extra]); // eslint-disable-line
  ...
  const exportCSV = () => {
    ...
    sortedCats.forEach(c => {
      ...
      csv += `"${c.id}","${c.name}","${c.description}","${cnt}","${coded.length > 0 ? ((cnt/coded.length)*100).toFixed(1) : "0.0"}%"\n`;
    });
    csv += "\nRESUMEN SENTIMIENTO\nSentimiento,Frecuencia,Porcentaje\n";
    SENT_OPTS.forEach(s => { const cnt = sentCounts[s]||0; csv += `"${s}","${cnt}","${coded.length>0?((cnt/coded.length)*100).toFixed(1):"0.0"}%"\n`; });
    ...
    const a = document.createElement("a"); a.href=url; a.download=`codificacion_${Date.now()}.csv`; a.click(); URL.revokeObjectURL(url);
  };
  ...
  const C = {
    ...
    stepTitle: { fontSize: 12, color: "#5a5a68", marginBottom: 18 },
  ...
  return (
    <div style={C.app}>
      ...
      <div style={C.inner}>
        ...
        <p style={C.sub}>Herramienta de investigación de mercados. Codificación automática de verbatims con IA. Libro de códigos inductivo, edición manual y exportación lista para reportes.</p>
        ...
        <div style={{ ...C.card, ...C.cardHL }}>
          ...
          <div style={C.info}>
            ...
            <span style={{ color: "#5a5a68", fontSize: 11 }}>✓ 1,500 requests/día gratis · ✓ Sin tarjeta · ✓ Activa en segundos</span>
          </div>
          <div style={{ display: "flex", gap: 10, marginTop: 14 }}>
            <input style={{ ...C.inp, flex: 1, letterSpacing: apiKey && !showKey ? ".15em" : "normal" }} type={showKey ? "text" : "password"} value={apiKey} onChange={e => saveKey(e.target.value)} placeholder="AIza..." />
        ...
        <div style={C.card}>
          ...
          <div style={C.row2}>
            <div><label style={C.label}>Pregunta / Tema analizado</label><input style={C.inp} value={question} onChange={e => setQuestion(e.target.value)} placeholder="Ej: ¿Por qué prefiere la marca Familia?" disabled={isRunning} /></div>
            <div><label style={C.label}>Sector / Categoría</label><input style={C.inp} value={sector} onChange={e => setSector(e.target.value)} placeholder="Ej: Papel higiénico, Consumo masivo" disabled={isRunning} /></div>
          </div>
          <div style={C.row2}>
            <div><label style={C.label}>Idioma</label>
              <select style={C.inp} value={lang} onChange={e => setLang(e.target.value)} disabled={isRunning}>
                <option value="español">Español</option><option value="inglés">Inglés</option><option value="portugués">Portugués</option>
              </select></div>
            <div><label style={C.label}>Máx. categorías</label>
          ...
          <div><label style={C.label}>Instrucciones adicionales (opcional)</label>
            <input style={C.inp} value={extra} onChange={e => setExtra(e.target.value)} placeholder="Ej: Distinguir razones funcionales de emocionales…" disabled={isRunning} /></div>
        </div>

        <div style={C.card}>
          <div style={C.badge()}>02 — VERBATIMS</div>
          <p style={C.stepTitle}>Una respuesta por línea. Pega directo desde Excel o cualquier fuente.</p>
          <textarea style={C.ta} value={rawText} onChange={e => setRawText(e.target.value)} disabled={isRunning}
            placeholder={"Pega aquí las respuestas...\n\nEjemplo:\nMe gusta porque es suave y no irrita\nEs económica y rinde más\nMuy buena calidad para el precio\n..."} />
          <div style={{ display: "flex", gap: 10, marginTop: 10, flexWrap: "wrap" }}>
            <span style={C.chip}>Respuestas: <span style={C.chipV}>{responses.length}</span></span>
          ...
          <p style={{ fontSize: 11, color: "#3a3a48", marginTop: 8 }}>💡 Lotes de {BATCH_SIZE} · pausa automática · reintenta si hay límite de API.</p>
        </div>

        <div style={C.card}>
          <div style={C.badge()}>03 — EJECUTAR</div>
          <p style={C.stepTitle}>Fase 1: construye el libro de códigos. Fase 2: codifica todos los lotes.</p>
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            {!isRunning
              ? <button style={{ ...C.btnP, opacity: responses.length === 0 || !apiKey ? 0.4 : 1 }} onClick={runCoding} disabled={responses.length === 0 || !apiKey}>⚡ Iniciar codificación</button>
              : <button style={C.btnR} onClick={cancel}>✕ Cancelar</button>}
          ...
          {isRunning && (
            <div style={{ marginTop: 20 }}>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: "#5a5a68", marginBottom: 5 }}>
                <span>{phase === "codebook" ? "Fase 1/2 — Construyendo libro de códigos" : `Fase 2/2 — Codificando (${pct}%)`}</span>
          ...
          {phase === "done" && <div style={C.ok}>✓ Completado — {coded.length} respuestas · {codebook.length} categorías</div>}
            ...
            {activeTab === "dashboard" && (
              <div>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(130px,1fr))", gap: 10, marginBottom: 28 }}>
                  {[{n:coded.length,l:"Total respuestas"},{n:codebook.length,l:"Categorías"},{n:sentCounts.POSITIVO,l:"Positivo",c:"#5af0c8"},{n:sentCounts.NEGATIVO,l:"Negativo",c:"#f05a5a"},{n:sentCounts.NEUTRO,l:"Neutro",c:"#888"},{n:sentCounts.MIXTO,l:"Mixto",c:"#f0c85a"}].map((k,i) => (
                ...
                <div style={C.sLabel}>Frecuencia por categoría</div>
                ...
                <div style={C.sLabel}>Libro de códigos</div>
            ...
            {activeTab === "table" && (
              <div>
                <div style={{ display: "flex", gap: 10, marginBottom: 14, flexWrap: "wrap", alignItems: "center" }}>
                  ...
                  <span style={{ ...C.chip, marginLeft: "auto" }}>Mostrando <span style={C.chipV}>{filteredCoded.length}</span> de {coded.length}</span>
                </div>
                <div style={C.tableWrap}>
                  <table style={C.table}>
                    <thead><tr>{["#","Respuesta original","Código(s)","Sentimiento","Notas IA"].map(h => <th key={h} style={C.th}>{h}</th>)}</tr></thead>
                    <tbody>
                      {filteredCoded.map((row, idx) => {
                        const isEditing = editingRow === row.id;
                        return (
                          <tr key={row.id} style={{ background: idx%2===0?"transparent":"rgba(255,255,255,.01)", cursor: "pointer" }} onDoubleClick={() => setEditingRow(isEditing ? null : row.id)}>
                                ...
                                <div>{codebook.map(c => {
                                  ...
                                  return (
                                    <label key={c.id} style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4, cursor: "pointer" }}>
                                      <input type="checkbox" checked={row.codes.includes(c.id)} onChange={e => updateRow(row.id, "codes", e.target.checked ? [...row.codes, c.id] : row.codes.filter(x => x !== c.id))} />
                            ...
                            <td style={{ ...C.td, minWidth: 100 }}>
                              {isEditing ? <select style={C.sel} value={row.sentiment} onChange={e => updateRow(row.id,"sentiment",e.target.value)}>{SENT_OPTS.map(s => <option key={s} value={s}>{s}</option>)}</select>
                            ...
                            <td style={{ ...C.td, fontSize: 11, color: "#5a5a68", maxWidth: 180 }}>
                              {isEditing ? <input style={{ ...C.inp, fontSize: 11, padding: "4px 8px" }} value={row.note||""} onChange={e => updateRow(row.id,"note",e.target.value)} /> : row.note}
                            </td>
                          </tr>
                        );
                      })}
                ...
                <p style={{ fontSize: 10, color: "#3a3a48", marginTop: 8 }}>💡 Doble clic en una fila para editar.</p>
              </div>
            )}
